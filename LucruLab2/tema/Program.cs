...
// Requirement 5: Static Lambda Filtering
Console.WriteLine("\nREQUIREMENT 5: Static Lambda Filtering");
StaticLambdaFiltering();
...
List<Book> GetUserBooks()
{
    ...
    while (true)
    {
        ...
        // Create a new Book record and add it to the list
...
// Step 5: Implements Static Lambda Filtering
void StaticLambdaFiltering()
{
    var allBooks = new List<Book>
    {
        new Book("The Great Gatsby", "F. Scott Fitzgerald", 1925),
        new Book("The Alchemist", "Paulo Coelho", 1988),
        new Book("The Name of the Wind", "Patrick Rothfuss", 2007),
        new Book("Project Hail Mary", "Andy Weir", 2021),
        new Book("Klara and the Sun", "Kazuo Ishiguro", 2021)
    };
    
    // Static Lambda Expression: The 'static' keyword prevents the lambda from capturing 
    // any local variables from its containing method, ensuring purity and potential performance gains
    Func<Book, bool> IsModern = static (book) => book.YearPublished > 2010;

    // Filtering the list using the LINQ Where method and the static lambda delegate
    var modernBooks = allBooks.Where(IsModern).ToList();

    Console.WriteLine("  - Books published after 2010 (Filtered):");
